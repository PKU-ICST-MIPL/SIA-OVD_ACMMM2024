...
class LvisDetection(TvLvisDetection):
    def __init__(self, img_folder, ann_file, transforms, return_masks, label_map, debug, repeat_factor_sampling=False, repeat_threshold=0.001, instancewise_sampling=False, balance_factor=0.5):
        ...
        self.instancewise_sampling = instancewise_sampling
        if instancewise_sampling:
            num_instances = len(self.lvis.dataset['annotations'])
            raw_category_weight = {k['id']: (k['instance_count'] + 1e-3) / num_instances for k in self.lvis.dataset['categories']}
            self.category_weight = {k: max(1.0, (repeat_threshold / v) ** balance_factor) for k, v in raw_category_weight.items()}
            rep_factors = []
            for id in self.ids:
                ann_ids = self.lvis.get_ann_ids(img_ids=[id])
                target = self.lvis.load_anns(ann_ids)
                cats = [t['category_id'] for t in target]
                rep_factor = sum([self.category_weight[cat_id] for cat_id in cats])
    ...
    def __getitem__(self, idx):
        ...
        if self.instancewise_sampling and len(target['annotations']) > 0:
            instance_weights = [self.category_weight[k['category_id']] for k in target['annotations']]
            s = sum(instance_weights)
            selected_instance = choice(target['annotations'], 1, p=[i / s for i in instance_weights])
            target['annotations'] = list(selected_instance)
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target):
        ...
        box_ids = [obj["id"] for obj in anno]
...
def build(image_set, args):
    ...
    PATHS = {
        "train": (root, root / "annotations/lvis_v1_train_norare.json"),
        "val": (root, root / "annotations/lvis_v1_val.json"),
    }

    if args.label_version == 'relabel':
        PATHS['train'] = (root, root / "annotations/lvis_v1_train_norare_relabel.json")
    ...
    dataset = LvisDetection(
        ...
        instancewise_sampling=args.instancewise_sampling and image_set == 'train',
