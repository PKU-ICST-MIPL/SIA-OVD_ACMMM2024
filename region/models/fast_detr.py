...
class WeightedAverage(nn.Module):
    def __init__(self):
        ...
        self.weight = nn.Parameter(torch.tensor(0.2))  # 初始化为 0.2

    def forward(self, x, output_feats):
        ratio = torch.sigmoid(self.weight)  # 使用 sigmoid 函数确保权重在 (0, 1) 范围内
...
class FastDETR(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, classifier, num_classes):
        ...
        # self.shape_adapter = ShapeAdapter(1024,4)
        # self.shape_adapter = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_xiaoyu1 = ShapeAdapter(1024,4)
        # self.shape_adapter_dayu1 = ShapeAdapter(1024,4)

        # self.shape_adapter_1 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_2 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_3 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_4 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_5 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_6 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_7 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_8 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_9 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_10 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_11 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_12 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_13 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_14 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_15 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_16 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_17 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_18 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_19 = nn.Linear(1024, 1024, bias=False)
        # self.shape_adapter_20 = nn.Linear(1024, 1024, bias=False)

        # self.weight_average = WeightedAverage()
        if args.backbone == "clip_RN50":
            if args.num_adapters >= 1:
                self.shape_adapter_1 = ShapeAdapter(1024,4)
            if args.num_adapters >= 2:
                self.shape_adapter_2 = ShapeAdapter(1024,4)
            if args.num_adapters >= 4:
                self.shape_adapter_3 = ShapeAdapter(1024,4)
                self.shape_adapter_4 = ShapeAdapter(1024,4)
            if args.num_adapters >= 10:
            ...
            if args.num_adapters >= 20:
                self.shape_adapter_11 = ShapeAdapter(1024,4)
                self.shape_adapter_12 = ShapeAdapter(1024,4)
                self.shape_adapter_13 = ShapeAdapter(1024,4)
                self.shape_adapter_14 = ShapeAdapter(1024,4)
                self.shape_adapter_15 = ShapeAdapter(1024,4)
                self.shape_adapter_16 = ShapeAdapter(1024,4)
                self.shape_adapter_17 = ShapeAdapter(1024,4)
                self.shape_adapter_18 = ShapeAdapter(1024,4)
                self.shape_adapter_19 = ShapeAdapter(1024,4)
                self.shape_adapter_20 = ShapeAdapter(1024,4)
            ...
            if args.num_adapters >= 20:
                self.shape_adapter_11 = ShapeAdapter(640,4)
                self.shape_adapter_12 = ShapeAdapter(640,4)
                self.shape_adapter_13 = ShapeAdapter(640,4)
                self.shape_adapter_14 = ShapeAdapter(640,4)
                self.shape_adapter_15 = ShapeAdapter(640,4)
                self.shape_adapter_16 = ShapeAdapter(640,4)
                self.shape_adapter_17 = ShapeAdapter(640,4)
                self.shape_adapter_18 = ShapeAdapter(640,4)
                self.shape_adapter_19 = ShapeAdapter(640,4)
                self.shape_adapter_20 = ShapeAdapter(640,4)
        else:
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        assert target_sizes.shape[1] == 2 # torch.tensor([[w, h]], device=device)
        ...
        img_h, img_w = target_sizes.unbind(1) # 维度1 分解为两个张量 w h
