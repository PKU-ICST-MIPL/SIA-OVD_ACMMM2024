...
class FastDETR(nn.Module):
    def __init__(self, args, backbone, transformer, classifier, num_classes):
        ...
        if region_prompt_path:
            ...
            shape_adapter_state = dict()
            for k, v in region_prompt.items():
                if 'shape_adapter' in k:
                    shape_adapter_state[k] = v.clone()

        if args.backbone == "clip_RN50":
            self.shape_adapter_1 = ShapeAdapter(1024,4)
            self.shape_adapter_2 = ShapeAdapter(1024,4)
            self.shape_adapter_3 = ShapeAdapter(1024,4)
            self.shape_adapter_4 = ShapeAdapter(1024,4)
            self.shape_adapter_5 = ShapeAdapter(1024,4)
            self.shape_adapter_6 = ShapeAdapter(1024,4)
            self.shape_adapter_7 = ShapeAdapter(1024,4)
            self.shape_adapter_8 = ShapeAdapter(1024,4)
            self.shape_adapter_9 = ShapeAdapter(1024,4)
            self.shape_adapter_10 = ShapeAdapter(1024,4)
        elif args.backbone == "clip_RN50x4":
            self.shape_adapter_1 = ShapeAdapter(640,4)
            self.shape_adapter_2 = ShapeAdapter(640,4)
            self.shape_adapter_3 = ShapeAdapter(640,4)
            self.shape_adapter_4 = ShapeAdapter(640,4)
            self.shape_adapter_5 = ShapeAdapter(640,4)
            self.shape_adapter_6 = ShapeAdapter(640,4)
            self.shape_adapter_7 = ShapeAdapter(640,4)
            self.shape_adapter_8 = ShapeAdapter(640,4)
            self.shape_adapter_9 = ShapeAdapter(640,4)
            self.shape_adapter_10 = ShapeAdapter(640,4)
        else:
            raise ValueError("Invalid CLIP backbone:" + args.backbone)

        adapter_1_state = dict()
        adapter_2_state = dict()
        adapter_3_state = dict()
        adapter_4_state = dict()
        adapter_5_state = dict()
        adapter_6_state = dict()
        adapter_7_state = dict()
        adapter_8_state = dict()
        adapter_9_state = dict()
        adapter_10_state = dict()

        adapter_1_state.update({k.replace('shape_adapter_1.', ''): v for k, v in shape_adapter_state.items() if k.startswith('shape_adapter_1.')})
        adapter_2_state.update({k.replace('shape_adapter_2.', ''): v for k, v in shape_adapter_state.items() if k.startswith('shape_adapter_2.')})
        adapter_3_state.update({k.replace('shape_adapter_3.', ''): v for k, v in shape_adapter_state.items() if k.startswith('shape_adapter_3.')})
        adapter_4_state.update({k.replace('shape_adapter_4.', ''): v for k, v in shape_adapter_state.items() if k.startswith('shape_adapter_4.')})
        adapter_5_state.update({k.replace('shape_adapter_5.', ''): v for k, v in shape_adapter_state.items() if k.startswith('shape_adapter_5.')})
        adapter_6_state.update({k.replace('shape_adapter_6.', ''): v for k, v in shape_adapter_state.items() if k.startswith('shape_adapter_6.')})
        adapter_7_state.update({k.replace('shape_adapter_7.', ''): v for k, v in shape_adapter_state.items() if k.startswith('shape_adapter_7.')})
        adapter_8_state.update({k.replace('shape_adapter_8.', ''): v for k, v in shape_adapter_state.items() if k.startswith('shape_adapter_8.')})
        adapter_9_state.update({k.replace('shape_adapter_9.', ''): v for k, v in shape_adapter_state.items() if k.startswith('shape_adapter_9.')})
        adapter_10_state.update({k.replace('shape_adapter_10.', ''): v for k, v in shape_adapter_state.items() if k.startswith('shape_adapter_10.')})

        self.shape_adapter_1.load_state_dict(adapter_1_state)
        for param in self.shape_adapter_1.parameters():
            param.requires_grad = False
        self.shape_adapter_2.load_state_dict(adapter_2_state)
        for param in self.shape_adapter_2.parameters():
            param.requires_grad = False
        self.shape_adapter_3.load_state_dict(adapter_3_state)
        for param in self.shape_adapter_3.parameters():
            param.requires_grad = False
        self.shape_adapter_4.load_state_dict(adapter_4_state)
        for param in self.shape_adapter_4.parameters():
            param.requires_grad = False
        self.shape_adapter_5.load_state_dict(adapter_5_state)
        for param in self.shape_adapter_5.parameters():
            param.requires_grad = False
        self.shape_adapter_6.load_state_dict(adapter_6_state)
        for param in self.shape_adapter_6.parameters():
            param.requires_grad = False
        self.shape_adapter_7.load_state_dict(adapter_7_state)
        for param in self.shape_adapter_7.parameters():
            param.requires_grad = False
        self.shape_adapter_8.load_state_dict(adapter_8_state)
        for param in self.shape_adapter_8.parameters():
            param.requires_grad = False
        self.shape_adapter_9.load_state_dict(adapter_9_state)
        for param in self.shape_adapter_9.parameters():
            param.requires_grad = False
        self.shape_adapter_10.load_state_dict(adapter_10_state)
        for param in self.shape_adapter_10.parameters():
            param.requires_grad = False
        # *************************************************************************
    ...
    def forward(self, samples: NestedTensor, categories, gt_classes=None, targets=None, split_class=False):
        ...
        if not self.training: # 测试
            ...
            if self.aux_loss and self.training:
                ...
                sample_box = boxes[-1:] # 1,2,1000,4 DETR最后一层输出的bbox

            for coord in sample_box: # bs,1000,4
                if self.args.box_conditioned_pe or hasattr(self, 'test_attnpool'):
                    box_emb = gen_sineembed_for_position(coord.permute(1,0,2))[:,:,256:]
                else:
                    box_emb = None
                
                # roi_feats.append(self._sample_feature(sizes, coord, src_feature.tensors, extra_conv=not self.training, box_emb=box_emb)) # no efficent pooling
                output_feat = self._sample_feature(sizes, coord, src_feature.tensors, extra_conv=not self.training, box_emb=box_emb) # 2,1000,1024
                
                # """
                #********************************************** adapter
                adapter_output_1 = self.shape_adapter_1(output_feat) # 2,1000,1024
                adapter_output_2 = self.shape_adapter_2(output_feat)
                adapter_output_3 = self.shape_adapter_3(output_feat)
                adapter_output_4 = self.shape_adapter_4(output_feat)
                adapter_output_5 = self.shape_adapter_5(output_feat)
                adapter_output_6 = self.shape_adapter_6(output_feat)
                adapter_output_7 = self.shape_adapter_7(output_feat)
                adapter_output_8 = self.shape_adapter_8(output_feat)
                adapter_output_9 = self.shape_adapter_9(output_feat)
                adapter_output_10 = self.shape_adapter_10(output_feat)

                true_boxes_xyxy = []
                boxes_xyxy = [box_cxcywh_to_xyxy(boxes.clone()) for boxes in coord]
                for box_idx, boxes in enumerate(boxes_xyxy):
                    w = targets[box_idx]['orig_size'][1] # tensor(640, device='cuda:0')
                    h = targets[box_idx]['orig_size'][0] # 480
                    scale_fct = torch.tensor([[w, h, w, h]], device=boxes.device) # tensor([[640, 426, 640, 426]], device='cuda:0')
                    scale_fct = scale_fct.expand(boxes.shape[0], -1) # 1000,4
                    true_boxes_xyxy.append(boxes * scale_fct)

                which_adapters = [] # ['1', '2', '1', '2', '2', '1', '2', '1'] 2000
                for boxes in true_boxes_xyxy:
                    which_adapter = []
                    for box in boxes:
                        x1 = box[0]
                        y1 = box[1]
                    ...
                    which_adapters.append(which_adapter)

                adapter_outputs_all = []
                for idx in range(output_feat.shape[0]):
                    adapter_outputs = []
                    for adapter_num, tensor_1, tensor_2, tensor_3, tensor_4, tensor_5, tensor_6, tensor_7, tensor_8, tensor_9, tensor_10 \
                        in zip(which_adapters[idx], adapter_output_1[idx], adapter_output_2[idx], adapter_output_3[idx], adapter_output_4[idx], adapter_output_5[idx], \
                            adapter_output_6[idx], adapter_output_7[idx], adapter_output_8[idx], adapter_output_9[idx], adapter_output_10[idx]
                                ):
                        # 根据适配器选择线性层
                        if adapter_num == '1':
                    ...
                    adapter_outputs_all.append(adapter_outputs)
                x = torch.stack(adapter_outputs_all, dim=0)

                ratio = 0.2
                output_feat = ratio * x + (1 - ratio) * output_feat
                #**********************************************
                # """
                
                roi_feats.append(output_feat) # no efficent pooling
    ...
    def _sample_feature(self, sizes, pred_boxes, features, extra_conv, unflatten=True, box_emb=None):
        ...
        if self.args.no_efficient_pooling or extra_conv: # no_efficient_pooling / inference 阶段
    ...
    def forward(self, outputs, targets):
        ...
        if 'split_class' not in outputs:
            ...
            try:
                ...
                print("ERROR!", e)
                print("losses['loss_bbox']:", losses['loss_bbox'])
                print("num_boxes:", num_boxes)
                print("targets['labels']:", [t["labels"] for t in targets])
